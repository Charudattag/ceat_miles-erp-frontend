...
const Exception = () => {
  ...
  const handleShowReplaceModal = (exception) => {
    ...
    const baseQuantity = exception.product_details?.product_quantity;
    setReplacementQuantity(baseQuantity);
  };

  const handleCloseReplaceModal = () => {
    setShowReplaceModal(false);
    setSelectedException(null);
    setReplacementQuantity(null);
  };
  ...
  const handlePlaceReplacementOrder = async () => {
    try {
      ...
      const orderAmount =
        selectedException.product_details.product_price * replacementQuantity;

      const payload = {
        cartItems: [
          {
            dealer_id: ticketDetails.data.dealer_code,
            product_id: ticketDetails.data.product_id,
            product_amount: orderAmount,
            product_quantity: replacementQuantity,
                  ...
                  <div className="d-flex gap-2">
                    ...
                    {exception.status === "APPROVED" && (
                      <Button
                        variant="success"
                        size="sm"
                        onClick={() => handleShowReplaceModal(exception)}
                      >
                        Replace
                      </Button>
                    )}
        ...
        <Modal.Body>
          <Form noValidate validated={validated}>
            <Form.Group className="mb-3">
              ...
              {validated && !newException.ticket_id && (
            ...
            <Form.Group className="mb-3">
              ...
              {validated && !newException.exception_type && (
                <small className="text-danger">This field is required</small>
              )}
            </Form.Group>
      ...
      <Modal show={showReplaceModal} onHide={handleCloseReplaceModal}>
        ...
        <Modal.Body>
          {selectedException && selectedException.product_details ? (
            <div>
              ...
              <p>
                Product Quantity:{" "}
                {selectedException.product_details.product_quantity}
              </p>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="number"
                  min={selectedException.product_details.product_quantity}
                  step={selectedException.product_details.product_quantity}
                  value={replacementQuantity}
                  onChange={(e) => {
                    const value = parseInt(e.target.value);
                    const baseQuantity =
                      selectedException.product_details.product_quantity;
                    if (value >= baseQuantity && value % baseQuantity === 0) {
              ...
              <div className="d-flex gap-2 mb-3">
                <Button
                  ...
                  onClick={() => {
                    const baseQuantity =
                      selectedException.product_details.product_quantity;
                    setReplacementQuantity((prev) =>
                      Math.max(baseQuantity, prev - baseQuantity)
                    );
                  }}
                >
                  -
                ...
                <Button
                  ...
                  onClick={() => {
                    const baseQuantity =
                      selectedException.product_details.product_quantity;
                    setReplacementQuantity((prev) => prev + baseQuantity);
                  }}
                >
                  +
                </Button>
              ...
              <p>
                Points per unit:{" "}
                {selectedException.product_details.product_price}
              </p>
              <p className="fw-bold">
                Total Points:{" "}
                {(
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            disabled={
              !selectedException?.product_details ||
              replacementQuantity <
                selectedException.product_details.product_quantity ||
              replacementQuantity %
                selectedException.product_details.product_quantity !==
